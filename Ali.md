...
# 链表
## 基本操作
- 非常重要！！！！反转链表的时候，最后一个的节点的next要注意处理，要么练到原本尾节点的下一个节点（如果不处理，很容易出现环形链表）
- 记得加一个dummynode
- 链表循环的时候，判断好while的退出条件，即是要当前结点还是前一结点
- 判断好边界条件的处理：比如空链表和单节点链表

## 合并升序列表
list1当前节点的值比list2当前节点的值小，说明上一个节点的next是list1，也就是当前小值的节点的下一个节点是递归该函数的返回节点
...
## [K个一组反转链表](https://leetcode.cn/problems/reverse-nodes-in-k-group/submissions/622120307/?envType=study-plan-v2&envId=top-100-liked)

不要搞的太复杂，先牺牲一点时间复杂度实现再说
思路：不断更新每k个的子链表的头节点，先判断接下来的子链表是否有k个节点，如果有，就反转，并把头节点和上个链表连接，并保存尾节点，供下一链表连接。
- 先实现一个字函数：给定头节点，和尾节点，反转这个子链表，并返回头节点、子节点
- 如果子链表不足k个元素，就不反转，直接将当前头节点和上一链表连接
- k=1的时候，需要注意一下，相当于不反转，而不是全部反转
- k=整个链表长度时，或者正好倍数关系，如果不处理最后一组链表的尾节点，会出现环形链表
- 反转链表的时候，最后一个的节点的next要注意处理，要么练到原本尾节点的下一个节点（如果不处理，很容易出现环形链表）
...
注意：层次遍历初始化或者输出的时候，一个空结点的下一层是不会用null占位置的，在下一层默认跳过两个位置

## 完全二叉树的结点个数  
递归实现：
- 完全二叉树如果不是一个满二叉树，可以不断向下走，直至子结点对应一个满二叉树
- 某节点如果是满二叉树，可直接根据公式返回节点数；如果不是满二叉树，再分别递归左结点和右结点；直至结点为满二叉树即可
- 判断是否为满二叉树：左遍历路径长度 = 右遍历路径长度
